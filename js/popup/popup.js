	...
	**/
	...
	Extension.port.onMessage.addListener(function (message, sender) {
		...
		**/
		...
		// Join storage copies with page selections.
		storage.copies = storage.copies.concat(message.copies);

		// Parse arguments from JSON.
		storage.copies = storage.copies.map(JSON.parse);
		...
		*
		**/
		for (let i = 0, l = storage.copies.length; i < l; i++) {

			document.body.insertNode('div', function (div) {
						...
						**/
						div.insertNode('div', { class: 'tp-xs-6 bp-xs-6' }, function (div) {
							div.insertNode('p', { class: 'font-xs-6 font-align-justify line-xs-12' }, function (p) {
								p.insertNode('span', storage.copies[i].text, { style: 'color: #232323; letter-spacing: 0.035rem; word-spacing: 0.1rem;' });
							});
						});
						...
						**/
						div.insertNode('div', { class: 'tp-xs-6 bp-xs-6' }, function (div) {
							div.insertNode('p', { class: 'font-xs-4 font-align-left font-style-italic line-xs-8' }, function (p) {
								p.insertNode('span', (storage.copies[i].meta.description ? storage.copies[i].meta.description : 'This site does not have a description. Feel free to add your own.'), { style: 'color: #232323; letter-spacing: 0.035rem; word-spacing: 0.1rem;' });
							});
						});
								...
								**/
								aside.insertNode('div', {class: 'flex-xs align-xs-center'}, function (div) {
									...
									div.insertSvgNode('svg', {x: '0px', y:'0px', viewBox:'0 0 50 50', 'xml:space':'preserve', class: 'tp-xs-2 rp-xs-2 bp-xs-2 lp-xs-2', style:'width: 1.4rem; height: 1.4rem;'}, function (svg) {
										...
										storage.copies[i].store = storage.copies[i].store === false ? true : false;
										
										let index = storage.copies.map(function (obj) { return obj.text }).indexOf(storage.copies[i].text);
										...
										div.insertNode('hgroup', { class: 'flex-xs align-xs-center' }, function (hgroup) {
											...
											**/
											hgroup.insertNode('h6', { class: 'tp-xs-2 rp-xs-2 bp-xs-2 lp-xs-2' }, function (h6) {
												h6.insertNode('span', { }, function (span) {
													span.insertNode('a', storage.copies[i].tab.host, { style: 'cursor: pointer; color: #0e6fff;'}).addEventListener('click', function () {
														Extension.browser.tabs.create({ url: 'http://' + storage.copies[i].tab.host})
													}, false);
												});
											});
											...
											**/
											hgroup.insertNode('h6', { class: 'tp-xs-2 rp-xs-2 bp-xs-2 lp-xs-2' }, function (h6) {
												...
												h6.insertNode('span', { class: 'font-weight-800', style: 'display: block;' }, function (span) {
													span.insertNode('a', storage.copies[i].tab.pathname, { style: 'display: block; cursor: pointer; color: #0e6fff;'}).addEventListener('click', function () {
														Extension.browser.tabs.create({ url: storage.copies[i].tab.href})
													}, false);
												});
											});
										});
	...
	Extension.port.onDisconnect.addListener(function () {

		Extension.browser.storage.sync.set({ [Extension.browser.name]:{ copies: ['SAMPLE', 'TEXT'] } })
