...
Extension.port.onMessage.addListener(function (message, sender) {
	...
	**/
	...
	Extension.browser.storage.sync.get(Extension.manifest.name, function (storage) {
		/**
		*** Fetch browser sync storage object.
		*
		* Asynchronous method.
		*
		**/
		...
		// Set storage array.
		storage.copied = storage.copied instanceof Array ? storage.copied : new Array();

		// Send message to event page.
		Extension.port.postMessage({ event: 'popup_page_reading_copies' });

		console.log('message:', message, 'storage:', storage);
		...
		document.body.insertNode('div', { class: 'tp-xs-6 rp-xs-6 bp-xs-6 lp-xs-6'}, function (div) {
			...
			**/
			for (let i = 0, l = message.copies.length; i < l; i++) {
				...
				div.insertNode('div', { class: 'col-xs-grow col-sm-10 rm-xs-auto bm-xs-4 lm-xs-auto', style: 'background: white; box-shadow: 0rem 4.5rem 4.0rem -4.5rem rgba(0, 0, 0, 0.25);', 'data-copy-object': JSON.stringify(message.copies[i]) }, function (div) {
								...
								**/
								aside.insertNode('div', {class: 'flex-xs align-xs-center'}, function (div) {
									...
									div.insertSvgNode('svg', {x: '0px', y:'0px', viewBox:'0 0 50 50', 'xml:space':'preserve', class: 'tp-xs-2 rp-xs-2 bp-xs-2 lp-xs-2', style:'width: 1.4rem; height: 1.4rem;'}, function (svg) {
										...
										*
										**/
										storage.copied.push(this.getAttribute('copy-object-data'));

										console.log('stored:', message.copies[i].text)
											...
											**/
											hgroup.insertNode('h6', { class: 'tp-xs-2 rp-xs-2 bp-xs-2 lp-xs-2' }, function (h6) {
												...
												h6.insertNode('span', { class: 'font-weight-800', style: 'display: block;' }, function (span) {
													span.insertNode('a', message.copies[i].tab.pathname, { style: 'display: block; cursor: pointer; color: #0e6fff;'}).addEventListener('click', function () {
